...
export function square (ctx, x, y, size, rotation = 0) {
    const test = polygon(4, size, x, y, rotation)
    const verticies = []
...
export function branch(ctx, len, counter, x0, y0, angle) {
  ...
  if (counter>4) {
    // laptop can't process more than 5 iterations with 4 branches
    ctx.restore()
    return
  }
  branch(ctx, 0.7*len, counter+1, 0, -len, angle)
  branch(ctx, 0.7*len, counter+1, 0, -len, -angle)
  branch(ctx, 0.7*len, counter+1, 0, -len, angle+30)
  branch(ctx, 0.7*len, counter+1, 0, -len, -angle-30)
  ctx.restore()
}